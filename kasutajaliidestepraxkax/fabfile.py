...
def defaults():
    ...
    env.service_name = ["gunicorn-kasutajaliidestepraxkax", ]
    env.code_dir = '/srv/kasutajaliidestepraxkax'

@task
def test():
    defaults()
    env.hosts = ['maximum.thorgate.eu']
    env.tag = 'kasutajaliidestepraxkax-test'
...
def live():
    ...
    env.tag = 'kasutajaliidestepraxkax-live'
...
def setup_server():
    ...
    # Create database
    sudo('echo "CREATE DATABASE kasutajaliidestepraxkax; '
         '      CREATE USER kasutajaliidestepraxkax WITH password \'%s\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE kasutajaliidestepraxkax to kasutajaliidestepraxkax;" '
    ...
    put(local_path=StringIO(local_settings), remote_path=env.code_dir + '/kasutajaliidestepraxkax/settings/local.py', use_sudo=True)

    # Create necessary dirs, with correct permissions
    mkdir_wwwdata('/var/log/kasutajaliidestepraxkax/')
    with cd(env.code_dir + '/kasutajaliidestepraxkax'), prefix('. ../venv/bin/activate'):
    ...
    sudo('chown -R www-data:www-data /var/log/kasutajaliidestepraxkax/')

    # Copy logrotate, nginx and gunicorn confs
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/kasutajaliidestepraxkax')
        sudo('cp deploy/%s /etc/nginx/sites-enabled/kasutajaliidestepraxkax' % env.nginx_conf)
        sudo('cp deploy/gunicorn.conf /etc/init/gunicorn-kasutajaliidestepraxkax.conf')
...
def collectstatic():
    with cd(env.code_dir + '/kasutajaliidestepraxkax'):
